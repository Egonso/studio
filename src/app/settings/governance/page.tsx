...
export default function GovernanceSettingsPage() {
        ...
        if (reg) {
          ...
          if (s) {
            ...
            setIncidentDocRequired(
              s.incidentConfig?.documentationRequired || false,
            );
            ...
            setEuAiActTraining(
              s.competencyMatrix?.euAiActTrainingRequired || false,
            );
            setTechAiCompetency(
              s.competencyMatrix?.technicalAiCompetency || false,
            );
            setPrivacyTraining(
              s.competencyMatrix?.dataPrivacyTraining || false,
            );
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: 'Governance-Einstellungen aktualisiert.',
      });
    } catch (error) {
  ...
  if (isLoading) {
    return (
      <SignedInAreaFrame
        ...
        description="Organisationsweite Vorgaben für Register, externe Erfassung und Review-Steuerung."
        nextStep="Wir laden Ihr Register und die vorhandenen Governance-Vorgaben."
      >
        <PageStatePanel
          tone="loading"
          area="paid_governance_control"
          title="Governance-Einstellungen werden geladen"
          description="Registerprofil, Rollen, Review-Logik und Zugangscodes werden vorbereitet."
        />
      </SignedInAreaFrame>
    );
  }
  ...
  if (!register) {
    return (
      <SignedInAreaFrame
        ...
        title="Governance-Einstellungen"
        description="Organisationsweite Vorgaben für Register, externe Erfassung und Review-Steuerung."
        nextStep="Erstellen Sie zuerst ein Register, damit Governance-Einstellungen gepflegt werden können."
      >
        <PageStatePanel
          area="paid_governance_control"
          title="Kein Register gefunden"
          description="Für dieses Konto wurde noch kein Register gefunden."
          actions={
            <Button asChild>
  ...
  return (
    <SignedInAreaFrame
      area="paid_governance_control"
      title="Governance-Einstellungen"
      description="Organisationsweite Vorgaben für Register, externe Erfassung und Review-Steuerung. Erweiterte Einstellungen wachsen mit Ihrem Bedarf."
      nextStep="Pflegen Sie zuerst Organisationsdaten, Rollen, Review-Logik und Zugangscodes."
      actions={
        <>
                ...
                <div className="space-y-3 p-4 rounded-md border bg-muted/20">
                  ...
                  <div className="flex items-center gap-3">
                    <Checkbox
                      ...
                      onCheckedChange={(c) => setRolesDefined(c === true)}
                    />
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    {SCOPE_OPTIONS.map((opt) => (
                      <div
                        key={opt.value}
                        className="flex items-center gap-3 p-3 rounded-md border bg-muted/10"
                      >
                        <Checkbox
                          id={`scope-${opt.value}`}
                ...
                <div className="space-y-3">
                  ...
                  {RACI_ROLES.map((role) => (
                    <RoleInput
                      ...
                      value={raci[role.key]}
                      onChange={(val) =>
                        setRaci((prev) => ({ ...prev, [role.key]: val }))
                      }
                    />
                  ))}
                </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-1.5">
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          setIncidentReportingPath(e.target.value)
                        }
                    ...
                    <div className="space-y-3">
                      <div className="flex items-center gap-3 p-3 rounded-md border bg-muted/10">
                        <Checkbox
                          id="iso-27001"
                          checked={iso27001}
                          onCheckedChange={(c) => setIso27001(c === true)}
                        />
                        <div>
                          <Label
                            htmlFor="iso-27001"
                          ...
                          <p className="text-xs text-muted-foreground">
                            Informationssicherheit – relevante Controls werden
                            im Audit-Export gemappt.
                          </p>
                        </div>
                      </div>
                      <div className="flex items-center gap-3 p-3 rounded-md border bg-muted/10">
                        <Checkbox
                          id="iso-42001"
                          checked={iso42001}
                          onCheckedChange={(c) => setIso42001(c === true)}
                        />
                        <div>
                          <Label
                            htmlFor="iso-42001"
