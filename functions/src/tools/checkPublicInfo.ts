...
// Move db init inside the function to avoid "default app not initialized" error
// const db = admin.firestore(); 

interface CheckPublicInfoData {
    projectId: string;
    toolId: string;
    toolIds?: string[]; // Future batch support
    force?: boolean;
}

export const checkPublicInfo = onCall({ secrets: [perplexityApiKey] }, async (request) => {
    const db = admin.firestore();
    ...
    const data = request.data as CheckPublicInfoData;
    ...
    // 3. Cache Check
    const lastCheckedAt = toolData?.publicInfo?.lastCheckedAt;
    ...
    let currentCount = usageData?.dailyCount || 0;

    if (usageData?.lastReset !== todayStr) {
        currentCount = 0;
    }
    ...
    `;
        ...
        if (resultData.sources && Array.isArray(resultData.sources)) {
            resultData.sources = resultData.sources.map((s: any) => ({
                ...s,
                accessedAt: now
            }));
        }
